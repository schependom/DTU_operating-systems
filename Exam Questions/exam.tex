...
\begin{answer}
    ...
    but with a different physical address space, even though the virtual address space is the same.
...
\begin{question}[4]
    The DMA chip can transfer data from and to the memory without using the CPU. Name a scenario that it is not eﬃcient to use DMA for memory transfer?
...
\begin{answer}
    ...
    This improves overall system throughput compared to \textit{Programmed I/O}.
    Compared to \textit{Interrupt-Driven I/O}, it reduces the number of interrupts
...
\begin{question}[4]
    Describe a bug or error that would trigger a watchdog timer reboot? Explain briey your answer.
\end{question}
...
\begin{answer}
    ...
    However, imagine we have an infinite loop in a kernel module: a deadlock
    or an infinite loop in the scheduler for example. Then the watchdog
    timer is not reset anymore, and it will eventually expire and trigger a reboot.

    Infinite loops in \textit{programs} do \textit{not} cause a watchdog timer reboot if we use a preamptive scheduler.
...
\begin{answer}
    ...
    All other OS services are run in user mode.
    ...
    In monolithic systems, however, the whole operating system runs in kernel mode.
    This means that in these systems, a bug in for example a driver can cause the entire system to crash,
    whereas in microkernel systems, drivers are usually run in user mode.

    Furthermore, an extension of a monolithic system requires the whole kernel to be rebuilt, while an extension of a microkernel system only requires the relevant modules to be rebuilt.
\end{answer}
...
\begin{answer}
    ...
    This allows us to run programs that are much bigger than the available physical memory,
...
\begin{answer}
    ...
    It is is needed when running multiple programmes without memory abstraction (i.e. without virtual memory).
    ...
    executable code upon compilation and makes it difficult to move processes in memory (e.g. during swapping) during runtime.

    Dynamic relocation \textit{does} allow programs to be loaded into different memory locations at runtime.

    Each process sees its own \textit{virtual memory space},
    and the OS doesn't need to change all memory addresses in the programmes. Instead, the address in the \textit{base register} gets added
    to every memory reference \textit{at runtime}. A \textit{limit register} ensures the
    process does not access memory outside its allocated block, eliminating the need for memory protection keys (like in static relocation).

    Crucially, the OS can easily \textit{move} processes (=swapping) in/out of physical memory simply by updating the registers.
    This is much more efficient than in static relocation.
\end{answer}
...
\begin{answer}
    ...
    requires clock interrupts to be able to signal the process when the alarm expires.
\end{answer}
...
\begin{question}[4]
    Nodes in a distributed system typically use protocols that have been standardised by interna-
    tional standardisation bodies to communicate with each other. Are international standards also necessary when
    nodes in a multicomputer system communicate with each other? Explain briey your answer
\end{question}
...
\begin{question}[4]
    ...
    \end{enumerate}
    For each example, explain briey why you chose the particular scheduling technique and how it benefits the
    application.
\end{question}
...
\begin{answer}
    ...
    The \texttt{waitpid(-1, NULL, 0)} call instructs the calling process to suspend execution (block) until \textit{any} one of its child processes terminates. This synchronization ensures that parents do not exit before their children, allowing the system to reap the child's exit status and prevent the creation of zombie processes.
...
\begin{answer} \begin{enumerate} \renewcommand{\labelenumi}{\roman{enumi}.}
        ...
        \item I would design a \textbf{Dispatcher-Worker} model with \textbf{User-Level Preemption} and a \textbf{Hybrid Queueing} system,
              ...
              \textbf{Prioritisation (User-Level Preemption):}\\
              ...
              to the score than a Priority 1 request and should be handled as soon as possible. Standard non-preemptive scheduling is unsafe because a
              long P1 task could block a P250 task. OS-level preemption (kernel threads) introduces context-switching overhead.
              Instead, I would implement \textbf{Voluntary Yielding}:
              \begin{itemize}
        ...
        \item The most efficient policy would be the \textbf{Clock Algorithm} (Second Chance).
              ...
              it requires moving a node to the front of a linked list on \textbf{every} cache hit.
...
\begin{answer}
    ...
    There are 4 CPU cores available, so I would use 1 dispatcher thread and \textbf{4 worker threads}. The dispatcher is I/O-bound: Its job is to \texttt{accept()}s a connection and read 49 bytes.
    This takes microseconds. For the vast majority of time, this thread is blocked (sleeping)
...
\begin{answer}
    ...
    \begin{lstlisting}[language=C]
    do {
        // 1. Check shared 'mutex' without locking bus
        if (mutex == 1) {
            delay();                // Exponential backoff
            status = 1;
        } else {
            // 2. Only try atomic TSL if mutex looks free
            lock_bus();
...
\begin{answer}
    ...
    This allows it to address two different memory locations or devices sequentially.

    Fly-by mode skips the intermediate step: it reads data and writes it directly from the source to the destination.
...
\begin{answer}
    ...
    If users were allowed to acquire a dedicated device directly, a user might hold onto it indefinitely, blocking everyone else. To solve this, spooling decouples the application from the device.

    User applications do not talk to the device directly. Instead, they place the data (e.g., a file to be printed) into a special spooling directory.
